...
function get-next-tag-ver {
    local del="${VERSIONING_DELIMITER:-.}"
	local base=$(get-current-git-tag | cut -d"$del" -f1)
	local ver=$(get-current-git-tag | cut -d"$del" -f2 | awk '{$NF+=1; print $0}')
	echo "${base}${del}${ver}"
}
...
function upload-release {
	github-release upload --user $ORG --repo $NAME \
		--tag $(get-next-tag-ver) --name $RELEASE_NAME --file $RELEASE_FILE
}
